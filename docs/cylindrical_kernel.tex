...
\title{Adaptive Spatial--Dynamical Affinity Blending\\for Macro-State Discovery in\\Successor Representation Agents}
\author{}
\date{}
...
\section{Cylindrical Affinity Kernel for Periodic State Spaces}
...
\subsection{Spectral Analysis of the Successor Representation}

Machado, Rosenbaum, Guo \& Liu~\cite{machado2018} proved that the
...
(options).  The earlier Laplacian framework by Machado, Bellemare \&
Bowling~\cite{machado2017} showed that eigenoptions correspond to
eigenvectors associated with the largest eigenvalues of the graph
Laplacian, providing a principled connection between spectral methods and
option discovery in reinforcement learning.

Our micro-level spectral clustering on $\mathbf{M}$ follows this
established approach directly.

\subsection{Clustering in Periodic Domains}

Clustering data embedded in spaces with periodic boundary conditions is a
well-studied problem in computational science.  Standard distance metrics
treat $\theta = -\pi$ and $\theta = +\pi$ as maximally separated,
producing artefacts at the domain boundary.  The established solution is
to embed periodic coordinates on the unit circle
$(\cos\theta, \sin\theta)$ so that the metric respects the underlying
topology.  This approach has been applied to $k$-means~\cite{gao2022} and
DBSCAN~\cite{lazzari2025} in molecular simulation and other periodic
settings.

\subsection{Multi-Source Affinity Combination}
...
``views'' of the same data (e.g.\ different feature spaces or
modalities).  Constrained spectral clustering modifies the affinity
matrix with must-link / cannot-link constraints or spatial regularisation
terms~\cite{vonluxburg2007}.  Our convex combination of a dynamics-based
affinity ($\widetilde{\mathbf{A}}^{\text{SR}}$) with a geometry-based
kernel ($\mathbf{K}^{\text{spatial}}$) is conceptually analogous,
treating the successor representation and the physical state layout as
two complementary views of the same state space.

\subsection{Novelty of the Present Approach}

To our knowledge, the specific combination of a \emph{learned successor
representation} (dynamics-based affinity) with a \emph{cylindrical
spatial RBF kernel} (geometry-based affinity respecting angle
periodicity) via an adaptive, confidence-weighted convex blend for
macro-state discovery has not appeared in prior work.  The method
synthesises three established ideas:
\begin{enumerate}
\item SR-based spectral clustering for option discovery
      (Machado~et~al., 2017; 2018),
\item Cylindrical embedding for periodic state spaces
      (Gao~et~al., 2022; Lazzari~et~al., 2025), and
\item Multi-source affinity fusion from multi-view clustering.
\end{enumerate}

The adaptive confidence weighting (Section~1.5) is, to our knowledge, a
new contribution that makes the blending robust to environments
containing dynamical barriers (walls), where a uniform spatial kernel
would incorrectly smooth over true bottleneck structure.

%% ====================================================================
\begin{thebibliography}{9}

\bibitem{machado2018}
Machado, M.~C., Rosenbaum, C., Guo, X., Liu, M., Tesauro, G., \&
Campbell, M. (2018).
...
\bibitem{machado2017}
Machado, M.~C., Bellemare, M.~G., \& Bowling, M. (2017).
...
\emph{Symmetry}, 14(6), 1237.
\url{https://doi.org/10.3390/sym14061237}

\bibitem{lazzari2025}
Lazzari, F., et~al. (2025).
