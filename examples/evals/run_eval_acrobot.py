...
from examples.configs import ACROBOT, SHARED
...
if __name__ == "__main__":
    # Acrobot configuration (from centralized config)
    n_theta_bins = ACROBOT["n_theta_bins"]
    n_dtheta_bins = ACROBOT["n_dtheta_bins"]
    n_clusters = ACROBOT["n_clusters"]
    gamma = ACROBOT["gamma"]
    nruns = ACROBOT["eval_n_runs"]
    eps = list(ACROBOT["eval_episodes"])
    ...
    if args_cli.quick:
        eps = list(ACROBOT["eval_quick_episodes"])
        nruns = ACROBOT["eval_quick_n_runs"]
    ...
    if os.path.exists(os.path.join(data_dir, "SR_rewards_hierarchy.npy")):
        ...
        plot_reward_curves(args.episodes, data, os.path.join(save_dir, "acrobot_reward.png"))
    ...
    if os.path.exists(os.path.join(data_dir, "SR_steps_hierarchy.npy")):
        ...
        if os.path.exists(q_path):
            data["Q-Learning"] = np.load(q_path)
        plot_step_curves(args.episodes, data, os.path.join(save_dir, "acrobot_steps.png"))
    ...
    plot_stability_bars(stability_data, os.path.join(save_dir, "acrobot_relative_stability.png"))
