...
Saves .npy data files to data/acrobot/ and figures to figures/eval_acrobot/.
...
def plot_acrobot_rewards(args, save_dir="figures/eval_acrobot"):
    """Plot reward curves with confidence bands (Hierarchy vs Flat)."""
    os.makedirs(save_dir, exist_ok=True)
    eps_range = args.episodes

    hier = np.load("data/acrobot/SR_rewards_hierarchy.npy")[:, :len(eps_range)]
    flat = np.load("data/acrobot/SR_rewards_flat.npy")[:, :len(eps_range)]
...
def plot_acrobot_steps(args, save_dir="figures/eval_acrobot"):
    """Plot steps-to-goal curves (Hierarchy vs Flat)."""
    os.makedirs(save_dir, exist_ok=True)
    eps_range = args.episodes

    hier = np.load("data/acrobot/SR_steps_hierarchy.npy")[:, :len(eps_range)]
    flat = np.load("data/acrobot/SR_steps_flat.npy")[:, :len(eps_range)]
...
def plot_acrobot_stability(args, save_dir="figures/eval_acrobot"):
    """Plot relative stability bar chart (Hierarchy vs Flat)."""
    os.makedirs(save_dir, exist_ok=True)

    hier_path = "data/acrobot/SR_relative_stability_hierarchy.npy"
    flat_path = "data/acrobot/SR_relative_stability_flat.npy"
...
if __name__ == "__main__":
    ...
    if args_cli.train:
        os.makedirs("data/acrobot/", exist_ok=True)

        # Save args
        with open("data/acrobot/args.json", "w") as f:
        ...
        # Save data
        np.save("data/acrobot/SR_rewards_hierarchy.npy", SR_rewards_hier)
        np.save("data/acrobot/SR_rewards_flat.npy", SR_rewards_flat)
        np.save("data/acrobot/SR_steps_hierarchy.npy", SR_steps_hier)
        np.save("data/acrobot/SR_steps_flat.npy", SR_steps_flat)
        np.save("data/acrobot/SR_relative_stability_hierarchy.npy", SR_rel_stability_hier)
        np.save("data/acrobot/SR_relative_stability_flat.npy", SR_rel_stability_flat)
        print("\nSaved all data to data/acrobot/")

    else:
        # Load saved args
        if os.path.exists("data/acrobot/args.json"):
            with open("data/acrobot/args.json", "r") as f:
                saved = json.load(f)
    ...
    os.makedirs("figures/eval_acrobot", exist_ok=True)

    if os.path.exists("data/acrobot/SR_rewards_hierarchy.npy"):
        plot_acrobot_rewards(args)

    if os.path.exists("data/acrobot/SR_steps_hierarchy.npy"):
        plot_acrobot_steps(args)

    plot_acrobot_stability(args)

    print("\nDone! Figures saved to figures/eval_acrobot/")
